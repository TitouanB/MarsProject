...
\begin{itemize}
\item The first one shows that small changes of green color on a point or inside the same point do not affect the color detection, but could have an impact on the centroid as it is weighted by the intensity of the color. This kind of noise may appears if the albedo of the target is not uniform for instance.
\item The second noise is a spot of a green-brown color. As it is expected, it is not detected by the color detection algorithm. It may happens, as for the third and fifth noises, if the target contains green minerals for example.
